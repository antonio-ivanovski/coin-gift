...
export const app = new Hono()
	...
	.post("/gifts", async (c) => {
		try {
			...
			const response: CreateGiftResponse = {
				...
				expires_at: new Date(Date.now() + (body.expires_in_days || 30) * 24 * 60 * 60 * 1000)
			};

			return c.json(response, { status: 201 });
		} catch (error) {
			return c.json({ success: false, message: "Failed to create gift" }, { status: 400 });
		}
	})
	...
	.post("/gifts/redeem", async (c) => {
		try {
			...
			return c.json({ success: false, message: "Failed to redeem gift" }, { status: 400 });
		}
	})
	...
	.get("/gifts/:secret/status", async (c) => {
		try {
			...
			return c.json({ success: false, message: "Gift not found" }, { status: 404 });
		}
	})
	...
	.get("/bitcoin/fees", async (c) => {
		try {
			...
			return c.json({ success: false, message: "Failed to fetch fees" }, { status: 500 });
		}
	})
	...
	.post("/admin/process-batch", async (c) => {
		try {
			...
			return c.json({ success: false, message: "Batch processing failed" }, { status: 500 });
		}
	});
